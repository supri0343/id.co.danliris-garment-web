...
{
    public class XlsWIPCommandHandlerTest : BaseCommandUnitTest
    {
        ...
        public XlsWIPCommandHandlerTest()
        {
        ...
        private GetXlsWIPQueryHandler CreateGetXlsWIPQueryHandler()
        {
            return new GetXlsWIPQueryHandler(_MockStorage.Object, serviceProviderMock.Object);
        }

        [Fact]
        public async Task Handle_StateUnderTest_ExpectedBehavior()
        {
            GetXlsWIPQueryHandler unitUnderTest = CreateGetXlsWIPQueryHandler();
            ...
            GetXlsWIPQuery getXlsWIPQuery = new GetXlsWIPQuery(1, 25, "{}", DateTime.Now.AddDays(5), "token");

            _mockGarmentPreparingItemRepository
                .Setup(s => s.Query)
                .Returns(new List<GarmentPreparingItemReadModel>
                {
                    new Domain.GarmentPreparings.GarmentPreparingItem(guidPreparingItem, 1, new Domain.GarmentPreparings.ValueObjects.ProductId(1), "1", "", "", 1, new Domain.GarmentPreparings.ValueObjects.UomId(1), "", "", 0, 50, guidPreparing,null,"bcno","bctype",DateTime.Now).GetReadModel()
                }.AsQueryable());
            ...
            _mockGarmentDeliveryReturnItemRepository
                ...
                {
                    new GarmentDeliveryReturnItem(guidDeliveryReturnItem,guidDeliveryReturn,1,1,guidPreparingItem.ToString(), new Domain.GarmentDeliveryReturns.ValueObjects.ProductId(1),"","","","",9, new Domain.GarmentDeliveryReturns.ValueObjects.UomId(1),"","color","rak","level","box","area").GetReadModel()
                }.AsQueryable());


            var result = await unitUnderTest.Handle(getXlsWIPQuery, cancellationToken);
