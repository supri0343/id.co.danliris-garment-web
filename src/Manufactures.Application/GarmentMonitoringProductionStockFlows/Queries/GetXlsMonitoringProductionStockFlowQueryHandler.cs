		...
		public async Task<MemoryStream> Handle(GetXlsMonitoringProductionStockFlowQuery request, CancellationToken cancellationToken)
		{
			...
			foreach (var item in querySum)
			{
				GarmentMonitoringProductionStockFlowDto garmentMonitoringDto = new GarmentMonitoringProductionStockFlowDto()
				{
					...
					EndBalancCuttingePrice = Math.Round(((Convert.ToDouble(item.fare) * 0.25) + (Convert.ToDouble(item.basicprice) * item.fc)) * (item.beginingcuttingPrice + item.priceCuttingIn - item.priceCuttingOut - item.priceCuttingTransfer - item.priceCuttingSubkon - item.priceavalcut - item.priceavalsew), 2),
					...
					EndBalanceLoadingPrice = Math.Round(((Convert.ToDouble(item.fare) * 0.25) + (Convert.ToDouble(item.basicprice) * item.fc)) * (item.beginingloadingPrice + item.priceLoadingIn + item.qtyloadingInTransfer - item.priceloading - item.priceLoadingAdj), 2),
