		...
		public async Task<MemoryStream> Handle(GetXlsMonitoringProductionStockFlowQuery request, CancellationToken cancellationToken)
		{
			...
			foreach (var item in querySum)
			{
				GarmentMonitoringProductionStockFlowDto garmentMonitoringDto = new GarmentMonitoringProductionStockFlowDto()
				{
					...
					EndBalancCuttingePrice = Math.Round(((Convert.ToDouble(item.fare) * 0.25) + (Convert.ToDouble(item.basicprice) * item.fc)) * (item.begining + item.qtyCuttingIn - item.qtycutting - item.qtyCuttingTransfer - item.qtCuttingSubkon - item.qtyavalcut - item.qtyavalsew), 2),
			...
			foreach (var report in garmentMonitoringProductionFlow.garmentMonitorings)
			{
				...
				else
				{
					reportDataTable.Rows.Add(report.Ro, report.Article, report.BuyerCode, report.Comodity, report.QtyOrder, report.FC, report.Hours, report.Fare, report.BasicPrice,
						report.BeginingBalanceCuttingQty, Math.Round(report.BeginingBalanceCuttingPrice,2), report.QtyCuttingIn, Math.Round(report.PriceCuttingIn,2), report.QtyCuttingOut, Math.Round(report.PriceCuttingOut,2), report.QtyCuttingTransfer, Math.Round(report.PriceCuttingTransfer,2), report.QtyCuttingsubkon, Math.Round(report.PriceCuttingsubkon,2), report.AvalCutting, Math.Round(report.AvalCuttingPrice,2), report.AvalSewing, Math.Round(report.AvalSewingPrice,2), report.EndBalancCuttingeQty, Math.Round(report.EndBalancCuttingePrice,2),
