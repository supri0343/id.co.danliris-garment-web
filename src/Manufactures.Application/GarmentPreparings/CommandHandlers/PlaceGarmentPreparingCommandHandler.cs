    ...
    public class PlaceGarmentPreparingCommandHandler : ICommandHandler<PlaceGarmentPreparingCommand, GarmentPreparing>
    {
        ...
        public async Task<GarmentPreparing> Handle(PlaceGarmentPreparingCommand request, CancellationToken cancellationToken)
        {
            ...
            //{
                ...
                request.Items.Select(x => new GarmentPreparingItem(Guid.NewGuid(), x.UENItemId, new ProductId(x.Product.Id), x.Product.Code, x.Product.Name, x.DesignColor, x.Quantity, new UomId(x.Uom.Id), x.Uom.Unit, x.FabricType, x.RemainingQuantity, x.BasicPrice, garmentPreparing.Identity,x.ROSource,x.BCNo,x.BCType,x.BCDate)).ToList()
