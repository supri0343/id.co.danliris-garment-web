	...
	public class GetMonitoringPrepareQueryHandler : IQueryHandler<GetMonitoringPrepareQuery, GarmentMonitoringPrepareListViewModel>
	{
		...
		public async Task<GarmentMonitoringPrepareListViewModel> Handle(GetMonitoringPrepareQuery request, CancellationToken cancellationToken)
		{
			...
			var QueryMutationPrepareItemsROASAL = (from a in QueryMutationPrepareNow
												   ...
												   select new { article = a.Articles, roJob = a.RO, buyerCode = a.Buyer, price = Convert.ToDecimal((from aa in sumbasicPrice where aa.RO == a.RO select aa.BasicPrice / aa.Count).FirstOrDefault()), rono = a.RO, roasal = b.ROSource });
            ...
            var QueryAval = from a in (from data in garmentAvalProductRepository.Query where data.AvalDate <= dateTo select new { data.Identity, data.RONo, data.AvalDate })
                            ...
                            join d in (from data in garmentPreparingRepository.Query where data.UnitId == request.unit select new { data.Identity, data.RONo }) on a.RONo equals d.RONo
            ...
            var QueryDeliveryReturn = from a in (from data in garmentDeliveryReturnRepository.Query where data.ReturnDate <= dateTo && data.UnitId == request.unit select new { data.RONo, data.Identity, data.ReturnDate })
            ...
            var queryNow = from a in (QueryMutationPrepareItemNow
						   ...
						   join b in QueryMutationPrepareItemsROASAL on a.rono equals b.rono
                           select new { a, b };
