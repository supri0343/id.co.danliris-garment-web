	...
	public class GetXlsPrepareQueryHandler : IQueryHandler<GetXlsPrepareQuery, MemoryStream>
	{
		...
		public async Task<MemoryStream> Handle(GetXlsPrepareQuery request, CancellationToken cancellationToken)
		{
			...
			var QueryMutationPrepareNow = from a in (from aa in garmentPreparingRepository.Query
													 where aa.UnitId == (request.unit == 0 ? aa.UnitId : request.unit) && aa.ProcessDate.Value.AddHours(7).Date <= dateTo.Date
													 select new
													 {
														 aa.Identity,
														 aa.Article,
			...
			var QueryCuttingDONow = from a in (from data in garmentCuttingInRepository.Query
											   where data.UnitId == (request.unit == 0 ? data.UnitId : request.unit) && data.CuttingInDate.AddHours(7).Date <= dateTo.Date
											   select new
											   {
												   data.RONo,
												   data.Identity,
